<div class="card-body table-responsive p-0 pb-2">
                ...
                <?php $i=$page;foreach($rows as $r){
                  ...
                  <tr>
                    ...
                    <td> <span class="changeStatus" data-toggle="change-status" value="<?=($r->status==1) ? 0 : 1?>" data="<?=$r->id?>,tb_notes,id,status" ><i class="<?=($r->status==1) ? 'fa-regular fa-circle-check' : 'fa-solid fa-circle-xmark'?>" title="Click for chenage status" style="color:<?=($r->status==1) ? 'green' :'red'?> "></i></span>
                    </td>
                     <td>
                    <a href="javascript:void(0)" data-toggle="modal" data-target="#showModal" data-whatever="Update Notes :-<?=$r->section_name?> ( <?=$r->sub_name?> )" data-url="<?=$update_url?><?=$r->id?>" title="Update">
