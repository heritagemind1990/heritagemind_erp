    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          ...
          <div class="col-sm-1">
          <a href="<?=base_url();?>teacher-student-marks-upload" class="btn btn-danger btn-sm float-right"><i class="fas fa-backward"></i>   Back</a>
          </div>
        
          <div class="col-sm-12">
          <hr style="border:1px solid green;">
          <?php $tea = $this->teacher_model->view_data_id('teacher_master',$map->tea_id);
                $student = $this->teacher_model->get_student($map->section_id);?>
           <div class="row">
            <div class="col-sm-3">
            <span>Teacher Name : <?=$tea->name;?></span> <br><span>Subject Name : <?=$map->sub_name;?></span>
            </div>
            <div class="col-sm-8">
            <form action="<?=base_url('teacher-student-marks-upload/marks_upload/'.$map->id);?>" method="post">
            <div class="col-sm-11">
              ...
              <div class="card-body table-responsive p-0 pb-2">
                <table class="table table-hover text-nowrap  table-bordered">
                  ...
                  <tbody>
                    ...
                    $exam = $this->teacher_model->getDataID('exam_master',$exam_id); ?>
                    <tr>
                      <td><?=$i;?></td>
                      <td><?=$stu->stu_id;?></td>
                      ...
                      <?php $marks = $this->teacher_model->getStudentMark($stu->id,$exam_id,$map->id,$map->section_id,$map->sub_id);;?>
                      <td><input type="number" name="marks[]" max="<?=$exam->max_marks;?>" min="0" oninput="checkValue(this);" class="form-control" placeholder="Enter Marks" <?php if($marks->student_id??''==$stu->id){;?> value="<?=$marks->marks;?>" <?php if($marks->is_locked=='lock'){echo 'readonly';}?> <?php }else{echo 'value="0"';} ;?> ></td>
                    </tr>
                    ...
                    <?php if($count1 ==1){;?>
                      <a class="btn btn-danger float-right mt-3 mb-4 mr-5">Locked</a>
